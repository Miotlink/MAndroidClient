 ...
 */
public class OrderToEvaluateFragment extends BaseListFragment<OrderEntity> implements GetOrderView, OrderActionView, HeaderViewLayout.OnRefreshListener, ClientOrderListAdapter.HostAction, BalancePayView {
    ...
    public static OrderToEvaluateFragment newInstance() {
        OrderToEvaluateFragment fragment = new OrderToEvaluateFragment();
        return fragment;
    }

    public OrderToEvaluateFragment() {
...
//        load();
    ...
    public void showError(Throwable e) {
        ...
        {
            OrderInfo orderInfo = PreferencesUtil.getObject(StaticData.NO_EVALUATE, OrderInfo.class);
            if (orderInfo != null)
    ...
    public void onLoadMore() {
        presenter.getMoreOrderList(StaticData.ORDER_LIST_TOEVALUATE);
    }
    ...
    public void getData(boolean isShowDialog) {
        if (PreferencesUtil.getBoolean(StaticData.USER_LOGIN_STATUE)) {//用户未登录显示空布局提示
            presenter.getOrderList(StaticData.ORDER_LIST_TOEVALUATE, isShowDialog);
        } else {
            render(null);
        }
    }
