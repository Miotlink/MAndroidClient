...
public class NewOutofdateRedPacketFragment extends BaseListFragment<CouponContents> implements CouponContentsView {

    @Inject
    CouponContentPresenter presenter;
    private NewRedPacketItemAdapter newRedPacketItemAdapter;

    public static NewOutofdateRedPacketFragment newInstance() {
        Bundle args = new Bundle();
        NewOutofdateRedPacketFragment fragment = new NewOutofdateRedPacketFragment();
        return fragment;
    }
...
//        presenter.getCouponList(2);
    ...
    public void onResume() {
        super.onResume();
        presenter.getCouponList(2, null, null, null, null, false);
    }
    ...
    public RecyclerView.Adapter initAdapter(List<CouponContents> list) {
        newRedPacketItemAdapter = new NewRedPacketItemAdapter(list, 2);
    ...
    public void showError(Throwable e) {
        if (e instanceof AuthException) {
            render(null);
            if (isShow) {
                mNavigator.login(NewOutofdateRedPacketFragment.this, REQUEST_LOGIN);
                isShow = false;
            }
        } else {
            super.showError(e);
