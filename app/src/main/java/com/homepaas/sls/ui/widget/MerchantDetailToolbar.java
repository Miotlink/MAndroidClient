 ...
 * @author zhudongjie .
 */
public class MerchantDetailToolbar extends Toolbar {

    private TextView mCenterTitle;

    public MerchantDetailToolbar(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs, defStyleAttr);
    }

    public MerchantDetailToolbar(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    ...
    private void init(final Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //标题栏公共配置
        setNavigationIcon(R.mipmap.return_white);
        //清单中设置就用清单，没设置就统一颜色
        setBackground(context.getResources().getDrawable(R.drawable.merchant_detail_top_bg));
        mCenterTitle.setTextSize(TypedValue.COMPLEX_UNIT_SP, 18);
        mCenterTitle.setTextColor(context.getResources().getColor(R.color.white));
