...
import com.homepaas.sls.ui.widget.LoopPagerAdapter;
...
//            if (((LoopPagerAdapter) pagerAdapter).getItemCount() != 0) {
//                super.setCurrentItem(midd - midd % ((LoopPagerAdapter) pagerAdapter).getItemCount(), false);
//            }
            super.setCurrentItem(getStartItem((LoopPagerAdapter) pagerAdapter));
        }
    }

    public void resetLoopForDataChanged() {
        //当使用LoopViewPager时,初始化无限循环的position为中间
        PagerAdapter pagerAdapter = getAdapter();
        if (pagerAdapter instanceof LoopPagerAdapter) {
//            int midd = pagerAdapter.getCount() / 2;
