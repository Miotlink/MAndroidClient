...
public class GetOrderToConfirmInteractor extends Interactor<OrderInfo> {

    OrderInfoRepo orderInfoRepo;

    @Inject
    public GetOrderToConfirmInteractor(@Named("concurrent") JobExecutor jobExecutor, PostExecutionThread postExecutionThread, OrderInfoRepo orderInfoRepo) {
    ...
    protected Observable<OrderInfo> buildObservable() {
        return Observable.create(new Observable.OnSubscribe<OrderInfo>() {
            ...
            public void call(Subscriber<? super OrderInfo> subscriber) {
                ...
                try {
                    orders = orderInfoRepo.getToConfirmOrder();
